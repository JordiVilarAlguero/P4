...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db_devel/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           EXEC="gmm_train -v 1 -T 0.0001 -N 80 -m 64 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train"
       ...
       EXEC="gmm_train -v 1 -i 1 -T 0.0001 -N 80 -m 60 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train"
       echo $EXEC && $EXEC || exit 1
